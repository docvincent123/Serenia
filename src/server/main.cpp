...
if(path=="/api/discharges"){allow(u,{"admin","psychologist"});check(method=="POST","Метод не дозволений",405);int pid=num(b,"patient_id");auto p=getPatient(db,u,pid);auto from=str(b,"date_from",10),to=str(b,"date_to",10);validDate(from);validDate(to);check(from<=to,"Перевірте період виписки");auto count=db.query("SELECT count(*) n FROM consultations WHERE patient_id=? AND substr(created,1,10) BETWEEN ? AND ?",{pid,from,to})[0]["n"];auto psychologistId=p["psychologist_id"];db.query("INSERT INTO discharge_summaries(patient_id,author_id,psychologist_id,summary,dynamics,recommendations,followup,consultation_count,date_from,date_to,created) VALUES(?,?,?,?,?,?,?,?,?,?,?)",{pid,u["id"],psychologistId,str(b,"summary",15000),str(b,"dynamics",15000,false),str(b,"recommendations",15000,false),str(b,"followup",15000,false),count,from,to,now()});int id=db.id();audit(db,u,"create","discharge",id);auto center=db.query("SELECT * FROM center_settings WHERE id=1")[0];return {{"id",id},{"consultation_count",count},{"patient",p},{"center",center}};}
